...
const { getInsecureTrip, getSecureTrip } = require(__dirname+'/tripcode.js')
	...
	, staffLevels = ['Admin', 'Mod'];

module.exports = async (inputName, permissions, boardSettings, __) => {

	const { forceAnon, defaultName } = boardSettings;
	const staffPermissions = [permissions.get(Permissions.ROOT), permissions.get(Permissions.MANAGE_GENERAL)];
	...
	let tripcode = null;
	let capcode = null;
	if ((permissions.get(Permissions.MANAGE_GENERAL) || !forceAnon) && inputName && inputName.length > 0) {
		...
		if (matches && matches.groups) {
			...
			if (groups.tripcode) {
				...
				if (!permissions.get(Permissions.MANAGE_GENERAL) && groups.capcode === '##' && !groups.capcodetext) {
			...
			//capcode
			if (permissions.get(Permissions.MANAGE_GENERAL) && groups.capcode) {
				...
				const staffLevel = staffLevelDirect || staffLevelFallback;
				capcode = staffLevel;
